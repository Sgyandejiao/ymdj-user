		...
		methods: {
			...
			save() {
				...
				if (this.id) {
					...
					let valueList = data.address.region.label.split('-')

					let params = {
						"address": data.address.detailed,
						"cityId": data.address.region.cityCode,
						"cityName": valueList[2],
						"districtId": data.address.region.cityCode,
						"districtName": valueList[1],
						"linkMobile": data.tel,
						"linkRealName": data.name,
						"provinceId": data.address.region.cityCode,
						"provinceName": valueList[0]
					}
					console.log('params=', params)
				
					this.$util.request(this.$apis.addAddress, params, 'POST').then(r => {
						console.log('增加地址', r)
						if (r.status) {
							this.$toast('添加成功')
							setTimeout(res => {
