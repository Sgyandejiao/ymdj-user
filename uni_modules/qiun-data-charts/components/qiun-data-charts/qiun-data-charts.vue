        ...
        for (let i in args) {
            for (let key in args[i]) {
                if (args[i].hasOwnProperty(key)) {
                    origin[key] = args[i][key] && typeof args[i][key] === 'object' ? deepCloneAssign(Array.isArray(args[
    ...
    export default {
        ...
        watch: {
            ...
            optsProps: {
                handler(val, oldval) {
                    if (typeof val === 'object') {
                        if (JSON.stringify(val) !== JSON.stringify(oldval) && this.echarts === false && this
                            .optsWatch == true) {
        ...
        methods: {
            beforeInit() {
                this.mixinDatacomErrorMessage = null;
                if (typeof this.chartData === 'object' && this.chartData != null && this.chartData.series !==
                    undefined && this.chartData.series.length > 0) {
                    ...
                    .boundingClientRect(data => {
                        if (data.width > 0 && data.height > 0) {
                            ...
                            if (this.echarts !== true) {
                                ...
                                cfu.option[cid].tapLegend = false;
                            }
                            ...
                            if (this.inH5 || this.inApp) {
                                ...
                                this.$nextTick(() => {
                                    if (this.type2d === true) {
                                        ...
                                        query
                                            ...
                                            .exec(res => {
                                                if (res[0]) {
                                                    ...
                                                    cfu.option[cid].rotateLock = cfu.option[cid].rotate;
                                                    if (cfu.instance[cid] && cfu.option[cid] && cfu
                                                        .option[cid].update === true) {
                                        ...
                                        if (cfu.instance[cid] && cfu.option[cid] && cfu.option[cid]
                                            .update === true) {
                                            this._updataUChart(cid)
                                        } else {
                                            setTimeout(() => {
                ...
                if (tc && tc !== undefined && tc !== null) {
                    ...
                    cfu.instance[cid].showToolTip(e, {
                        ...
                        formatter: (item, category, index, opts) => {
                            if (typeof cfu.option[cid].tooltipFormat === 'string' && cfu.formatter[cfu
                                    .option[cid].tooltipFormat]) {
...
<script module="rdcharts" lang="renderjs">
    ...
    function rddeepCloneAssign(origin = {}, ...args) {
        for (let i in args) {
            for (let key in args[i]) {
                if (args[i].hasOwnProperty(key)) {
                    origin[key] = args[i][key] && typeof args[i][key] === 'object' ? rddeepCloneAssign(Array.isArray(
                        args[i][key]) ? [] : {}, origin[key], args[i][key]) : args[i][key];
                }
            }
        }
        return origin;
    ...
    export default {
        ...
        methods: {
            ...
            ecinit(newVal, oldVal, owner, instance) {
                ...
                if (newData) {
                    //挂载categories和series
                    if (cfe.option[cid].xAxis && cfe.option[cid].xAxis.type && cfe.option[cid].xAxis.type ===
                        'category') {
                        cfe.option[cid].xAxis.data = newData.categories
                    }
                    if (cfe.option[cid].yAxis && cfe.option[cid].yAxis.type && cfe.option[cid].yAxis.type ===
                        'category') {
                        cfe.option[cid].yAxis.data = newData.categories
                    }
                    cfe.option[cid].series = []
                    for (var i = 0; i < newData.series.length; i++) {
                        cfe.option[cid].seriesTemplate = cfe.option[cid].seriesTemplate ? cfe.option[cid]
                            .seriesTemplate : {}
